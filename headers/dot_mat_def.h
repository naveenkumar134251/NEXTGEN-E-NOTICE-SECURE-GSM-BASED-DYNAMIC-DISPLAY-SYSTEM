/**
 * @file    dot_mat_def.h
 * @brief   8×8 LED font bitmap table and row-pin definition for the dot-matrix display.
 *
 * Contains:
 *   1. ROWS macro — the GPIO bit position (P0.16) used as the starting
 *      bit for the 8 row control lines via the 74HC573 latch.
 *
 *   2. font[][] array — a 96-entry lookup table covering printable ASCII
 *      characters from ' ' (space, 0x20) through '~' (tilde, 0x7E).
 *      Each entry holds 8 bytes, one per display row.
 *
 * Indexing convention:
 *   font[ascii_code - 32][row]
 *   Example: font['A' - 32][0] = first row bitmap of the letter 'A'.
 *
 * Bitmap format:
 *   Each byte represents one horizontal row of an 8×8 character cell.
 *   Bits are active-low — a '0' bit lights the corresponding LED column.
 *   Bit 7 corresponds to the leftmost column (COL1), bit 0 to COL8.
 *
 * Coverage:
 *   Index  0 → ' ' (space)     0x20
 *   Index  1 → '!'             0x21
 *   ...
 *   Index 94 → '~'             0x7E
 *   (96 entries total; indices 95 is unused/padding in some compilers)
 */

#ifndef DOT_MAT_DEF_H
#define DOT_MAT_DEF_H

/**
 * @brief  Starting GPIO bit position for the 8 row control lines on Port 0.
 *
 * Row lines are driven on P0.16–P0.23 via the 74HC573 latch.
 * WRITEBYTE(IOPIN0, ROWS, value) writes `value` to bits [23:16] of IOPIN0.
 */
#define ROWS 16

/**
 * @brief  8×8 active-low font bitmap table for 96 printable ASCII characters.
 *
 * Usage: font[character - 32][row_index]
 * Each byte = one row of the character; bit 7 = leftmost column (active low).
 */
